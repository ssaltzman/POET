...
class ReportFindings(BaseView):
  ...
  -->  
    <script src="''' + join(WEB_PROGRAM_URL, 'crir.js') + '''"></script>
    <link href="''' + join(WEB_PROGRAM_URL, 'crir.css') + '''" rel="stylesheet" type="text/css"/>
    ...
    request.writeln('<body onLoad="makeChart();">')
	...
	function openProgInfo() {
        ...
        function redirectToCSV(name){
          ...
          window.open("''' + WEB_PROGRAM_URL + '''/"+name+".xls");
        }
    ...
    for s in sets:
      request.writeln('''
		      ...
		      <input name="set-filter" id="'''+s.name+'''" type="checkbox" value="'''+s.name+'''" checked="checked" class="crirHiddenJS" />						
    ...
    request.writeln('''
		...
		<input name="poet-filter" id="None" type="checkbox" value="None" checked="checked" class="crirHiddenJS" />
	      </div>
  ...
  def get_findings_action(self, request, filterChoice):
    ...
    for q in questions:
      ...
      #log.info("filterChoice[1] = "+str(filterChoice[1]))
      #log.info("poet = "+str(poet))
      # check poet
      if 'All' in filterChoice[1]:
        #log.info("All found -- includePoet TRUE")
        includePoet = True
      elif 'None' in filterChoice[1] and not poet: #"not poet" => "poet == []" => "question has no poet factors"
        #log.info("N/A filter on AND no Question has no POET factors -- includePoet TRUE")
        includePoet = True        
      else:
        includePoet = False
        for filterPoet in filterChoice[1]: 
          if filterPoet in poet:
            #log.info("match found between filterChoice[1] and poet -- includePoet TRUE")
            includePoet = True
            break
      #if not includePoet:
      #  log.info("includePoet FALSE")
  ...
  def makeQLists(self, questions, filters):
    ...
    # Imports: filters (group_filter, poet_filter, sets_filter)
    # Exports: qLists
    ...
    group_filter, poet_filter, sets_filter = filters
  ...
  def makeDictionaries(self, meeting, filters, returnAll):
    ...
    answerData = {} # Exposes individual user answers. The complete raw data, no processing or obfuscation. 
    answerDataAveraged = {} # As above, but user answers are averaged. Obfuscates individual responses. 
    questionPOETData = {} 
    questionSetData = {}
    poetData = {}
    setData = {}
    ...
    groupsList = [] #groupsList is not used until step 3
    userDictionary = {} #userDictionary is not used until step 2
    for group in groups:
      groupsList.append(group.name)
      userDictionary[group.name] = []
      for user in group:
        userDictionary[group.name].append(user.user_id)
    qLists = ReportFindings.makeQLists(self, questions, filters)

    #log.info("BEGIN: makeDictionaries")
    for q in questions:  
      if q.delete:
        continue

      """ This is all just for debugging. For each question, neatly prints who answered it
      log.info("-------------------------")
      q_answers = q.search1(name='answers')
      if not type(q_answers) == type(None):
        log.info(q.text+" ("+q.id+")")
        for a in q_answers:
          log.info(" * "+a.who+": "+a.answer)
      """
            ...
            for qg in q_groups: #belongs to:
              if user_id in userDictionary[qg]: #ignore the groups the user doesn't belong to
                ...
                if not qpindex in qp_add: 
                  ...
                  #log.info("questionPOETData["+qpindex+"] = "+str(questionPOETData[qpindex]))
                  qp_add.append(qpindex)
                
                qsindex = str(q.id)+"|"+qs
                if not qsindex in qs_add:
                  try:
                  ...
                  qs_add.append(qsindex)

                pindex = qp + "|" + qg
                if not pindex in p_add:
                  try:
    ...
    if returnAll:
      return (answerData, answerDataAveraged, questionPOETData, questionSetData, poetData, setData)
    else:
      return setData

  def makeExports(self, dictionaries):
    events = []
    groupsList = ["PM", "PMO", "Contractor", "Senior Stakeholder", "User"]
    answerData, answerDataAveraged, questionPOETData, questionSetData, poetData, setData = dictionaries 
    ...
    for aset in sets:
      ...
      if len(allData) > 0:
        ...
        tempCSV += ",Total,0,0\n\n"
      tempCSV += ",Question id,Text,Mean,Std Dev, Responders, Responses\n"
      for key in questionSetData.keys():
        qid, qset = key.split("|")
        if qset == aset:
    ...
    events.append(Event('redirectToResults'))                  
