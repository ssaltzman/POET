...
class POET(BaseView.BaseView):
  ...
  def send_admin_page(self, request):
    ...
    for group in groups:
      for user in group:
	 emailAddresses += str(Directory.get_user(user.user_id).email + ";")

    if not request.session.user.superuser == '1':
      request.writeln('''<a href="mailto:''' + emailAddresses.rstrip(';') + '''?subject=Survey%20Questions%20Released&body=The%20questions%20in%20the%20survey%20has%20been%20released%20to%20you.%20Please%20login%20to%20take%20the%20survey.">''')
    ...
    for group in groups:
      ...
      request.writeln('<td>')      
      if group.sets == '':
        request.writeln(request.cgi_form(_mhaction='openQuestions', groupid=group.id, setsid=sets.id, name='switchSets_'+ group.name))
        setList = []
        for item in sets:
	...
	request.writeln('''<div class="setControls"><input type="image" class="closedSet" src="'''+join(WEB_PROGRAM_URL, 'switch.png')+'''" value="closed" alt="Submit" name="switchSet" id="switch_'''+ group.name + '''" onclick='document.getElementById("switch_'''+ group.name+'''").style.marginLeft="0px";document.getElementById("switch_'''+ group.name+'''").value="released";' /></div>''')
        ...
        request.writeln(request.cgi_form(_mhaction='closeQuestions', groupid=group.id, setsid=sets.id, name='switchSets_'+ group.name))
        setList = group.sets
