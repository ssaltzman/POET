...
class Administrator(BaseView.BaseView):  
  ...
  def users_page(self, request):
	...
	request.writeln('''<span style="display:none;" id="userinfo_''' + user.id + '''">''' + user.id + '''</span>
			...
			<script type="text/javascript">
			  users["''' + user.id + '''"] = {
			    ...
			    "office":"''' + user.getvalue('office','') + '''",
			    "work":"''' + user.getvalue('work','') + '''",
			    "home":"''' + user.getvalue('home','') + '''",
			    "mobile":"''' + user.getvalue('mobile','') + '''",
			    "fax":"''' + user.getvalue('fax','') + '''",
			    "comments":"''' + user.getvalue('comments','') + '''"
			    };
			</script>
			
			<span style="float:right;">
			  <a class="ui-icon ui-icon-pencil" onclick='editUser("''' + user.id + '''");' href='#'></a>
			  <a class="ui-icon ui-icon-closethick" onclick='deleteUser("''' + user.id + '''");' href='#'></a>
			</span>
		      </div>
	''')
    title = "Edit User"
    request.writeln('''
      ...
      ''' + request.cgi_form(_adminaction='saveuser', userid=None, name=None, email=None, username=None, password=None, title=None, office=None, work=None, home=None, mobile=None, fax=None, comments=None) + '''
      <input id="dialogUserid" name="userid" type="hidden" value="''' + user.getvalue('userid', '') + '''" />
      <table border=0 cellspacing=5><tr>
        <td>Real Name:</td>
        <td><input id="dialogName" type="text" name="name" size="30" value="''' + user.getvalue('name', '') + '''"></td>
      </tr><tr>
        <td>Email:</td>
        <td><input id="dialogEmail" type="text" name="email" size="30" value="''' + user.getvalue('email', '') + '''"></td>
      </tr><tr>
        <td>Username:</td>
        <td><input id="dialogUsername" type="text" name="username" size="30" value="''' + user.getvalue('username', '') + '''"></td>
      </tr><tr>
        <td>Password:</td>
        <td><input id="dialogPassword" type="text" name="password" size="30" value="''' + user.getvalue('password', '') + '''"></td>
      </tr><tr>
        <td>Title:</td>
        <td><input id="dialogTitle" type="text" name="title" size="30" value="''' + user.getvalue('title' ,'') + '''"></td>
      </tr><tr>
        <td>Office:</td>
        <td><input id="dialogOffice" type="text" name="office" size="30" value="''' + user.getvalue('office', '') + '''"></td>
      </tr><tr>
        <td>Work Number:</td>
        <td><input id="dialogWork" type="text" name="work" size="30" value="''' + user.getvalue('work', '') + '''"></td>
      </tr><tr>
        <td>Home Number:</td>
        <td><input id="dialogHome" type="text" name="home" size="30" value="''' + user.getvalue('home', '') + '''"></td>
      </tr><tr>
        <td>Mobile Number:</td>
        <td><input id="dialogMobile" type="text" name="mobile" size="30" value="''' + user.getvalue('mobile', '') + '''"></td>
      </tr><tr>
        <td>Fax Number:</td>
        <td><input id="dialogFax" type="text" name="fax" size="30" value="''' + user.getvalue('fax', '') + '''"></td>
      </tr><tr>
        <td valign="top">Comments:</td>
        <td><textarea id="dialogComments" name="comments" rows=10 cols=50>''' + user.getvalue('comments', '') + '''</textarea></td>
      </tr>
      <tr>
