...
import time
import cgi #file upload
import os #for finding current working directory, can be removed later
  ...
  def send_content(self, request):
    ...
    request.writeln('''
      <script language='JavaScript' type='text/javascript'>
		...
		$("#importDialog").dialog({height: 330, width: 600, modal: true, autoOpen: false});
		$("#importBut").click(function() { $("#importDialog").dialog("open"); });
		$("#closeImportDialog").click(function() { $("#importDialog").dialog("close"); });		
	});
	  ...
	  var setChecked = false;
	    ...
	    poetChecked = true;
          ...
          if (id == '') {
            ...
            alert("Please select a user group before saving the question.");
          }
	  else if(!setChecked){
            alert("Please select a set before saving the question.");
          }
	  else if(document.getElementById('questionTextInput').value == ""){
	    alert("Please enter text for the questions before saving the question.");
	  }
	  else if(document.getElementById('formatInput').value == null){
	    alert("Please select an answer format for the questions before saving the question.");
	  }
          else{
            ...
            if( document.getElementById('formatInput').value == 'topn'){
              sendEvent('edit_question', id, document.getElementById('questionTextInput').value, document.getElementById('descriptionInput').value, users, document.getElementById('formatInput').value, comment, optional, document.getElementById('numselections').value, checkedCategories, checkedPoet, sets, na);
            }else{
              sendEvent('edit_question', id, document.getElementById('questionTextInput').value, document.getElementById('descriptionInput').value, users, document.getElementById('formatInput').value, comment, optional, 0, checkedCategories, checkedPoet, sets, na);
            }
          }
        }
	...
	function togglePOET(checked){
          document.getElementById('pol').checked = checked;
	  document.getElementById('opt').checked = checked;
	  document.getElementById('econ').checked = checked;
	  document.getElementById('tech').checked = checked;
        }
        ...
        function populateForm(text, descrip, users, ansFormat, comment, comOpt, num, options, tags, na) {
          //Quesiton Form
          ...
          if(document.getElementById('na').checked){
            ...
            document.getElementById('naInput').checked = true;
          }else{
            document.getElementById('naInput').style.display = "none";
            document.getElementById('naInput').checked = false;
          }
        ...
        function redirectXML(){
           window.open("''' + WEB_PROGRAM_URL + '''/exportedMeeting.xml");
        }
	
	function importQuestions(){
	  var filecontent = document.getElementById('importFile').value; //IMPORT_ERROR: This is where it fails.
	  //var fname = document.getElementById('filename').value; //this works but is just the name of the filen
	  //sendEvent('import', filename);
          //alert("importQuestions() called.");
	  sendEvent('import', filecontent);
	}
          ...
          <table id="question-editor-columns">
            <tr>
                ...
                <div id="question-list" class="module">
                  ...
                  <div id="questionListContent">
                    <div class="top-toolbar">
                      ...
                      <input type="button" value="Import (Text)" id="importBut" />
                      <input type="button" value="Import (File)" onclick="javascript:sendEvent('import', 'null')" />
                    </div>
              ...
              <td id="col2" rowspan="2">
                  ...
                  <div class="module">
                    ...
                    <div id="questionDetailContent">
                      ...
                      <table class="form">
                        <tr>
                          <td class="label">
                            <label for="questionTextInput">Text <font color="red">*</font> :</label>
                          </td>
                          <td class="value">
			    ...
			    <br/><font size="2" >Select: <a href="javascript:void(0)" onclick="togglePOET(true)">all</a>&nbsp;<a href="javascript:void(0)" onclick="togglePOET(false)">none</a></font>
                          </td>
                          <td class="value">
                            <div id="poetInput">
                        ...
                        <tr>
                          <td class="label">
                            <label for="setInput">Tags/Sets <font color="red">*</font> :</label>
                          </td>
                          <td class="value">
                            <div id="setInput">
                        ...
                        <tr>
                          <td class="label">
                            <label for="userGroupInput">User Group <font color="red">*</font> :</label>
                        ...
                        <tr>
                          <td class="label">
                            <label for="formatInput">Answer Format <font color="red">*</font> :</label>
                          </td>
                          <td class="value">
                            ...
                            &nbsp;N/A Option <input type="checkbox" value="N/A" id="na" />
                          </td>
                        </tr>
                        <tr>
                          <td>&nbsp;</td>
		      ...
		      <span><font color="red">* Required</font>
	...
	<div id="importDialog" align="center" style="display:none;margin:5px;" title="Import Questions - Plain Text">
	<!--
	  <form method='POST' enctype='multipart/form-data' action="''' + CGI_BIN + '''/Upload.py">
	   ''' + request.cgi_multipart_form(_mhaction='importfile', action='Upload.py', filename=None) + ''' 
	...
	-->
	  <table cellspacing="4" align="center">
	    <tr><td>
	    <textarea name="importFile" id="importFile" rows="10" cols="50"></textarea>
	
	

    ''')

    request.writeln('''</td></tr>
	    <tr><td><input type="button" name="submit" id="closeImportDialog" value="Import"  onclick="importQuestions()" /></td></tr>
	  </table>
	  <!-- /form -->
	</div> <!-- /#import-dialog -->
      </div><!-- /#container -->     
...
# to do:
# 1) Add ability to choose where file is exported
# 2) Add ability to choose which file is imported
  def import_action(self, request, fileContent):
    log.info("entering import_action")
    ...
    if fileContent == "null":
      log.info("found null, attempting to open uploaded.xml")
      fileContent = open("../webroot/GroupMind/uploaded.xml")
      doc = xml.dom.minidom.parse(fileContent)

    else:
      log.info("found string, attempting to parse string")
      doc = xml.dom.minidom.parseString(fileContent)

    """
    log.info("REQUEST = " + str(request))
    for property, value in vars(request).iteritems():
      log.info(str(property)+": "+str(value))
      if property in ["form", "request", "env"]:
        for subprop, subvalue in vars(value).iteritem():
          log.info("   "+str(subproperty)+": "+str(subvalue))	
    #log.info("fName= " + str(request.getvalue("fileName",'nofile')))
    #log.info("FILENAME= " + str(request.form))

    
#   uncomment this if you want Import to delete all existing questions
    for q in questions: 
      q.delete = True
      q.save()
    ...
    #importfile = open("../webroot/GroupMind/exportedMeeting02072011.xml")
    importfile = open("importedQuestions.xml") #name comes from upload.cgi script, probably incorrect path
 	
    
    log.info("about to get form_data")
    form_data = request.form
    log.info("form_data received, value set to: "+str(form_data))
    log.info("about to get file_data")
    file_data = form_data['gm_arg1'].value
    log.info("file_data received, value set to: "+str(file_data))
    """

    #log.info("parsing file_data " + str(fileContent))
    #doc = xml.dom.minidom.parse(file_data)
    #doc = xml.dom.minidom.parseString(fileContent)
    expectedAttributes = ["comment", "comOpt", "name", "format", "descrip", "text", "users", "delete"]      
    for node1 in doc.childNodes:
      for node2 in node1.childNodes:
        for node3 in node2.childNodes:
          for questionNode in node3.childNodes:
            if questionNode.nodeName == "child": # we've found a question!
              # initialize the question and its children:
              newQuestion = datagate.create_item(creatorid=creator.id, parentid=questions.id)
              poet_item = datagate.create_item(creatorid=creator.id, parentid=newQuestion.id)
              poet_item.name = "poet"
              set_item = datagate.create_item(creatorid=creator.id, parentid=newQuestion.id)
              set_item.name = "sets"
              options_item = datagate.create_item(creatorid=creator.id, parentid=newQuestion.id)
              options_item.name = "options"
              options_item.num_selections = 0
              answers_item = datagate.create_item(creatorid=creator.id, parentid=newQuestion.id)
              answers_item.name = "answers"
              answers_item.save()
              
              # initialize the question's attributes: (probably not necessary, as long as the input file is valid, but why take chances?)
              for attr in expectedAttributes:
                setattr(newQuestion, attr, "") #newQuestion.attr = ""
              # set the question's attributes:
              for subnode in questionNode.childNodes:
                if subnode.nodeName == "data": # is this an attribute?
                  attribute = subnode.getAttribute("name") #yes! which attribute is it?
                  value1 = subnode.firstChild.nodeValue #and what is its value?
                  if attribute == "delete":
                    if value1[2] == "1":
                      newQuestion.delete = True
                    else:
                      newQuestion.delete = False #stored as a binary instead of a string, gets its own line of code
                  elif attribute == "users":
                    value2 = QuestionEditor.parseQuotes(self, value1)
                    newQuestion.users = value2 #a list instead of a string, uses all of value2 
                  elif (attribute == "text") or (attribute == "descrip"): 
                    value2 = QuestionEditor.parseText(self, value1) 
                    setattr(newQuestion, attribute, value2) #user-input string, requires special handling
                  elif not (attribute == "creatorid" or attribute == "parentid" or attribute == "id"): #these are internal GroupMind values and should _never_ be imported
                    value2 = QuestionEditor.parseQuotes(self, value1)
                    setattr(newQuestion, attribute, value2[0]) #value2[0] because the value is a single item
      
                elif subnode.nodeName == "child": # it's not an attribute. Is it a child?
                  childName = QuestionEditor.getNodeName(self, subnode) 
                  if childName == "poet":
                    poetFactors = QuestionEditor.getNodeChildren(self, subnode) # list of the names of subnode's children (i.e.: the question's poet factors)
                    for poetFactor in poetFactors: #for each poet factor...
                      newpoet = datagate.create_item(creatorid=creator.id, parentid=poet_item.id) #make a node, attach it to poet_item
                      newpoet.name = poetFactor #add its name
                      newpoet.save()
                  elif childName == "sets":
                    setFactors = QuestionEditor.getNodeChildren(self,subnode) #get the names of subnode's children (i.e.: the question's sets)
                    for setFactor in setFactors: #for each set...
                      newset = datagate.create_item(creatorid=creator.id, parentid=set_item.id) #make a node, attach it to set_item
                      newset.name = setFactor #add its name
                      newset.save()
                  elif childName == "options":
                    for dataNode in subnode.childNodes:
                      if dataNode.getAttribute("name") == "num_selections": #find the child,
                        rawvalue = dataNode.firstChild.nodeValue    #get its value,
                        parsedvalue = rawvalue[1:-2] #strip formatting characters
                        options_item.num_selections = int(parsedvalue) #and save it as an int
                        options_item.save()
...
# Input: The XML string representation of an item or a list
# Output: A list containing the values from the input, with all formatting removed
  def parseQuotes(self, string):
    quoteString = string.replace("\\'", "\"") #escaped ' marks are replaced with " marks
    splitString = quoteString.split("'") #the string is split based on unescaped ' marks
    quotes = []
    while len(splitString) > 1:
      quotes.append(str(splitString[1])) #str() translates from unicode
      splitString = splitString[2:] #drops the first two items
    return quotes

# Input: The XML string representation of user-inputted text
# Output: The string of that text, with formatting removed and escaped quote marks parsed
  def parseText(self, string):
    reducedString = string[2:-6] #drop the formatting characters
    if ('"' in reducedString) and ("'" in reducedString): #if both ' and " were used, Python will escape ' marks...
      escapeString = reducedString.replace("\\'", "'") #...so we unescape them, then return the string.
      return escapeString
    return reducedString #otherwise, Python is smart enough to avoid escape characters, and we can return as is.

# Input: A node with a <data name="name"> child.
# Output: The value of that <data name="name"> child, or empty string if it can't be found.
  def getNodeName(self, node):
    for dataNode in node.childNodes:
      if dataNode.getAttribute("name") == "name": #find the child,
        value1 = dataNode.firstChild.nodeValue    #get its value,
        value2 = QuestionEditor.parseQuotes(self,value1) #parse it
        return value2[0]                          #and return it
    log.info("ERROR: getNodeName failed to find name. Returning ''.")
    return ""

# Input: A node that originally had multiple children, each containing a "name" string.
# Output: A list of those names.
  def getNodeChildren(self, node):
    returnList = []
    for subnode in node.childNodes: 
      if subnode.nodeName == "child": #for each of node's "child"ren...
        for childNode in subnode.childNodes:
          if childNode.getAttribute("name") == "name": #find the "name" child of that child
            returnList.append(QuestionEditor.parseQuotes(self, childNode.firstChild.nodeValue)[0]) #add it to the list
            break #we've got this child's name, we can move onto the next child of "node"
    return returnList
  ...
  def export_action(self, request):
    ...
    #this returns four "Data" (all internal metadata of the "Question" node; discardable)...
    #...and some "Child" nodes (these are the questions: parse these!)
    #f = open('qaDoc.xml','w')
    #//print >>f, doc.toxml()
    #f.close()
    ...
    f = open("../webroot/GroupMind/exportedMeeting.xml", 'w')

    """
    print >> f, "#!/usr/bin/python"
    print >> f, "# HTTP Header"
    print >> f, '''Content-Type:application/octet-stream; name=\"exportedMeeting.xml"\r\n'''
    print >> f, '''Content-Disposition: attachment; filename=\"exportedMeeting.xml"\r\n\n''' 
    print >> f, '''<HEAD>'''
    print >> f, '''<META http-equiv="Content-Type" content="application/octet-stream; name=exportedMeeting.xml">'''
    print >> f, '''<META http-equiv="Content-Disposition" content="attachment; filename=exportedMeeting.xml">'''
    print >> f, '''</HEAD>'''
    ...
    #fileName = tkFileDialog.asksaveasfilename(initialfile="exportedMeeting.xml", title="Save Questions As...")
    
    #print filename
    
    events.append(Event('redirectXML'))
    return events
