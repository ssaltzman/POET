  ...
  def send_content(self, request):
    ...
    request.writeln('''
      <script language='JavaScript' type='text/javascript'>
		...
		$( "#importDialog" ).dialog({height: 200, width: 350, modal: true, autoOpen: false});
                ...
                <div id="question-list" class="module">
                  ...
                  <div id="questionListContent">
                    <div class="top-toolbar">
                      ...
                      <input type="button" value="Import" id="importBut" />
                    </div>
	...
	<div id="importDialog" align="center" style="display:none;margin:5px;" title="Import Questions">
	  <form>
    ...
    request.writeln('''</td></tr>
	    <tr><td><input type="button" id="closeImportDialog" value="Import" onclick="javascript:importQuestions()" /></td></tr>
	  </table>
	  </form>
	</div> <!-- /#import-dialog -->
      </div><!-- /#container -->     
...
# 2) Add ability to choose which file is imported
  def import_action(self, request, filename):
    root = datagate.get_item(request.getvalue('global_rootid', ''))
    questions = root.search1(name="questions")
    creator = request.session.user
#   uncomment this if you want Import to delete all existing questions
    ...
    doc = xml.dom.minidom.parse(importfile)
            ...
            if questionNode.nodeName == "child": # we've found a question!
              ...
              for subnode in questionNode.childNodes:
                if subnode.nodeName == "data": # is this an attribute?
                  attribute = subnode.getAttribute("name")
                  value1 = subnode.firstChild.nodeValue # yes! what attribute is it?
                  if attribute == "delete": 
